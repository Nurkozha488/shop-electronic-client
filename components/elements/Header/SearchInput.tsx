...
import { getPartByNameFx, searchPartsFx } from '@/app/api/electronicParts'
  ...
  const searchPart = async (search: string) => {
    try {
      ...
      const data = await searchPartsFx({
        url: '/electronic-parts/search',
        search,
      })

      const names = data
        .map((item: IElectronicPart) => item.name)
